import md5 from 'md5'
import CryptoJS from '../cypto/cryptojs_aes.js'

const reg = new RegExp('-', 'g')

export function getConnectUrl (): {url: string, body: string} {
  const e = h()
  const t = 4
  const n = parseInt(`${new Date().getTime() / 1e3}`)
  const i = S()
  const r = 0
  const a = guid().replace(reg, '')
  const o = ''
  const s = ''
  const c = {
    appid: e,
...
export function getTokenUrl (): {collect: string, mid: string, url: string} {
  const n = h()
  const i = 4
...
function signatureParam (e: any, t: any) {
  const n = []
  for (const i in e) {
